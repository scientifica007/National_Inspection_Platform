...
class AccountManager(UserManager):
    ...
    When no Person is supplied, one is created as a convenience for
    administrative setup; the caller can never attach an unrelated existing
    Person implicitly.

    The automatic Person and the Account are written in one atomic block. A
    failure anywhere in Account creation therefore rolls the automatic Person
    back instead of orphaning it. A Person supplied by the caller is never
    saved or deleted here: it was not created by this operation, so it is not
    this operation's to undo.
    """
    ...
    def _create_user(self, username, email, password, **extra_fields):
        ...
        autogenerated_person = None
        if person is None:
            person = Person(display_name=display_name)
            person.full_clean()
            autogenerated_person = person
        extra_fields["person"] = person

        with transaction.atomic():
            if autogenerated_person is not None:
                autogenerated_person.save()
...
class CapabilityGrant(models.Model):
    ...
    granted_by_account = models.ForeignKey(
        ...
        help_text="الحساب الذي أصدر المنحة. مطلوب دائمًا: الفاعل يبقى معروفًا.",
    )
